...
- 현재 단계: 00-06 완결. raw 품질 진단·처리 방침 확정 → 세션·세션상품·구매여정 마트 생성·raw 대조 검증(a-c 통과) → 같은 세션 strict 3단계 퍼널 → 세션 간 구매 에피소드 재구성·가설 점검·A/B 테스트 설계까지 완료. 05는 06의 사전 진단으로 유지했다. 대시보드(Tableau) 미생성 — 후속 작업.
